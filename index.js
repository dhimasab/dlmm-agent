...
import { config, reloadScreeningThresholds, computeDeployAmount, computeDeployAmountUsdc } from "./config.js";
...
import { getLastBriefingDate, setLastBriefingDate, getTrackedPosition, getTrackedPositions, setPositionInstruction, updatePnlAndCheckExits, queuePeakConfirmation, resolvePendingPeak, queueTrailingDropConfirmation, resolvePendingTrailingDrop, minutesOutOfRange } from "./state.js";
...
const DEPLOY = isUsdcMode ? config.management.deployAmountUsdc : config.management.deployAmountSol;
const DEPLOY_USDC = isUsdcMode ? config.management.deployAmountUsdc : 0;
...
/**
 * Auto-Sweep: swap excess SOL (> capitalSol) → USDC when no open positions.
 * Uses data already fetched by management cycle — zero extra RPC calls.
 *
 * @param {number} walletSol - Current wallet SOL balance (preMgmtSol from management)
 * @param {Array} livePositions - On-chain positions from getMyPositions()
 */
async function runAutoSweepSolToUsdc(walletSol, livePositions) {
  if (!config.management.autoSweepSolToUsdc) return;

  const SWEEP_STATE_PATH = repoPath("autoSweepSolToUsdc-state.json");
  let sweepState = {};
  try { sweepState = JSON.parse(fs.readFileSync(SWEEP_STATE_PATH, "utf8")); } catch {}

  // Today in WIB (UTC+7)
  const now = new Date();
  const wibMs = now.getTime() + 7 * 60 * 60 * 1000;
  const todayWIB = new Date(wibMs).toISOString().slice(0, 10);

  // Already swept today → no RPC needed
  if (sweepState.lastSweepDate === todayWIB) return;

  const capitalSol = config.management.capitalSol ?? 15;

  // Check open positions from management cycle data (already fetched, zero extra RPC)
  const openCount = (livePositions || []).filter((p) => !p.closed).length;
  if (openCount > 0) {
    log("auto-sweep", `Waiting — ${openCount} position(s) still open`);
    return;
  }

  // Wallet balance check (already fetched by management cycle)
  if (!walletSol || walletSol <= capitalSol + 0.005) return;

  const excessSol = parseFloat((walletSol - capitalSol).toFixed(4));
  if (excessSol < 0.01) return;

  log("auto-sweep", `Swapping ${excessSol} SOL → USDC ...`);
  const result = await swapToken({ input_mint: "SOL", output_mint: config.tokens.USDC, amount: excessSol });
  if (!result.success) { log("sweep_error", `Swap failed: ${result.error}`); return; }

  sweepState.lastSweepDate = todayWIB;
  sweepState.lastSweepAt = new Date().toISOString();
  sweepState.lastSweepAmountSol = excessSol;
  sweepState.lastSweepAmountUsdc = result.amount_out ?? 0;
  sweepState.lastSweepTx = result.tx;
  fs.writeFileSync(SWEEP_STATE_PATH, JSON.stringify(sweepState, null, 2) + "\n");

  log("auto-sweep", `SUCCESS! ${excessSol} SOL → ${result.amount_out ?? "?"} USDC | Tx: ${result.tx}`);
...
export async function runManagementCycle({ silent = false } = {}) {
  ...
  try {
    ...
    // Snapshot SOL before position sync — detect delta from auto-closed positions
    let preMgmtSol = 0;
    try {
      const preBal = await getWalletBalances({});
      preMgmtSol = preBal.sol;
    } catch (_) { /* best effort */ }
    const livePositions = await getMyPositions({ force: true }).catch(() => null);
    positions = livePositions?.positions || [];

    // ── SOL delta check after state sync ─────────────────────────
    if (isUsdcMode && preMgmtSol > 0) {
      try {
        const currentBal = await getWalletBalances({});
        const solDelta = Math.max(0, currentBal.sol - preMgmtSol);
        if (solDelta > 0.01) {
          log("cron", `SOL delta after position sync: ${solDelta.toFixed(3)} SOL — swapping to USDC`);
          ...
          if (swapRes?.success) {
            log("cron", `Auto-swapped ${solDelta.toFixed(3)} SOL → USDC after auto-close: ${swapRes.tx}`);
          } else {
            log("cron_error", `SOL delta swap failed: ${swapRes?.error}`);
          }
        }
      } catch (e) {
        log("cron_error", `SOL delta swap error: ${e.message}`);
      }
    }
    // ── End SOL delta check ──────────────────────────────────────
    ...
    // ── Auto-Sweep SOL → USDC ──────────────────────────────────
    try {
      await runAutoSweepSolToUsdc(preMgmtSol, positions);
    } catch (e) {
      log("cron_error", `Auto-sweep error: ${e.message}`);
    }
    // ── End auto-sweep ─────────────────────────────────────────
    ...
    // ── Build JS report ──────────────────────────────────────────────
    for (const p of positionData) {
      p.minutes_out_of_range = minutesOutOfRange(p.position);
    }
...
export async function runScreeningCycle({ silent = false } = {}) {
  ...
  try {
    ...
    if (!isDryRun) {
      if (isUsdcMode) {
        const minUsdc = config.management.minUsdcToOpen || config.management.deployAmountUsdc || 0;
        if (preBalance.usdc < minUsdc) {
          log("cron", `Screening skipped — insufficient USDC (${preBalance.usdc.toFixed(2)} < ${minUsdc} needed for deploy)`);
          screenReport = `Screening skipped — insufficient USDC (${preBalance.usdc.toFixed(2)} < ${minUsdc} needed for deploy).`;
          appendDecision({
            type: "skip",
            actor: "SCREENER",
            summary: "Screening skipped",
            reason: `Insufficient USDC (${preBalance.usdc.toFixed(2)} < ${minUsdc})`,
          });
          _screeningBusy = false;
          return screenReport;
        }
        // Ensure enough SOL for tx fees — no USDC→SOL top-up, reserve stays untouched
        if (preBalance.sol < 0.005) {
          log("cron", `Screening skipped — insufficient SOL for tx fees (${preBalance.sol.toFixed(4)} < 0.005)`);
          screenReport = `Screening skipped — insufficient SOL for tx fees (${preBalance.sol.toFixed(4)} < 0.005).`;
          appendDecision({
            type: "skip",
            actor: "SCREENER",
            summary: "Screening skipped",
            reason: `Insufficient SOL for tx fees (${preBalance.sol.toFixed(4)} < 0.005)`,
          });
          _screeningBusy = false;
          return screenReport;
        }
      } else {
        const minRequired = config.management.deployAmountSol + 0.005;
        if (preBalance.sol < minRequired) {
          log("cron", `Screening skipped — insufficient SOL (${preBalance.sol.toFixed(3)} < ${minRequired} needed for deploy + tx fees)`);
          screenReport = `Screening skipped — insufficient SOL (${preBalance.sol.toFixed(3)} < ${minRequired} needed for deploy + tx fees).`;
          appendDecision({
            type: "skip",
            actor: "SCREENER",
            summary: "Screening skipped",
  ...
  try {
    ...
    let deployAmount, deployAmountUsdc;
    if (isUsdcMode) {
      deployAmountUsdc = computeDeployAmountUsdc(currentBalance.usdc);
      const solPrice = currentBalance.sol_price || 1;
      deployAmount = deployAmountUsdc / solPrice;
      log("cron", `Computed deploy amount: ${deployAmountUsdc} USDC ≈ ${deployAmount.toFixed(3)} SOL (wallet: ${currentBalance.usdc} USDC, SOL price: $${solPrice})`);
    } else {
      deployAmount = computeDeployAmount(currentBalance.sol);
...
SCREENING CYCLE
${strategyBlock}
Positions: ${prePositions.total_positions}/${config.risk.maxPositions} | ${isUsdcMode ? `USDC: ${currentBalance.usdc.toFixed(2)} | SOL (gas reserve): ${currentBalance.sol.toFixed(3)} | Deploy budget: ${deployAmountUsdc} USDC (auto-swapped to SOL at execution)` : `SOL: ${currentBalance.sol.toFixed(3)} | Deploy: ${deployAmount} SOL`}
...
function formatWalletStatus(wallet, positions) {
  ...
  if (isUsdcMode) {
    const deployAmountUsdc = computeDeployAmountUsdc(wallet.usdc);
    lines.push(`USDC: ${wallet.usdc}`);
    lines.push(`Open positions: ${positions.total_positions}/${config.risk.maxPositions}`);
    lines.push(`Next deploy amount: ${deployAmountUsdc} USDC`);
  } else {
    const deployAmount = computeDeployAmount(wallet.sol);
    lines.push(`Open positions: ${positions.total_positions}/${config.risk.maxPositions}`);
    lines.push(`Next deploy amount: ${deployAmount} SOL`);
  }
  lines.push(`Dry run: ${process.env.DRY_RUN === "true" ? "yes" : "no"}`);
  return lines.join("\n");
}

function formatConfigSnapshot() {
  const deployLine = isUsdcMode
    ? `Deploy: ${config.management.deployAmountUsdc} USDC | gasReserve: ${config.management.gasReserve} SOL | maxPositions: ${config.risk.maxPositions}`
...
function formatHelpText() {
  return [
    ...
    "/sweepsol — auto-sweep SOL → USDC history & config",
...
async function telegramHandler(msg) {
  ...
  if (text === "/sweepsol") {
    try {
      const sweepState = JSON.parse(fs.readFileSync(repoPath("autoSweepSolToUsdc-state.json"), "utf8"));
      const enabled = config.management.autoSweepSolToUsdc ? "✅ Enabled" : "❌ Disabled";
      const capital = config.management.capitalSol ?? 15;
      let msg = `💰 **Auto-Sweep SOL → USDC**\n`;
      msg += `Status: ${enabled} | Capital: ${capital} SOL\n`;
      if (sweepState.lastSweepDate) {
        msg += `\n**Last sweep:** ${sweepState.lastSweepDate}\n`;
        msg += `Time: ${new Date(sweepState.lastSweepAt).toLocaleString("id-ID", { timeZone: "Asia/Jakarta" })}\n`;
        msg += `Amount: \`${sweepState.lastSweepAmountSol} SOL\` → \`${sweepState.lastSweepAmountUsdc} USDC\`\n`;
        msg += `Tx: \`${sweepState.lastSweepTx?.slice(0, 20)}...\`\n`;
      } else {
        msg += `\nNo sweep history yet.\n`;
      }
      await sendMessage(msg);
    } catch (e) {
      await sendMessage(`Sweep status not available: ${e.message}`).catch(() => {});
    }
    return;
  }
