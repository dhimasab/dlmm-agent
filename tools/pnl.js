...
let _pnlFallbackConnection = null;

export function getPnlConnection() {
  // Primary: pump.helius-rpc.com (config.pnl.rpcUrl)
  if (!_pnlConnection) {
...
export function getPnlFallbackConnection() {
  // Fallback: solana-rpc.publicnode.com (RPC_FALLBACK_URL)
  if (!_pnlFallbackConnection) {
    const fallbackUrl = process.env.RPC_FALLBACK_URL || "https://solana-rpc.publicnode.com";
    _pnlFallbackConnection = new Connection(fallbackUrl, "confirmed");
  }
  return _pnlFallbackConnection;
}

// Patch getLatestSig to retry with fallback on 429
const _originalGetLatestSig = getLatestSig;
// Wrap with fallback: try primary, if 429 → fallback
export async function getLatestSigWithFallback(conn, addr) {
  try {
    const sigs = await conn.getSignaturesForAddress(new PublicKey(addr), { limit: 1 });
    return sigs?.[0]?.signature ?? null;
  } catch (e) {
    if (e?.message?.includes("429") || e?.message?.includes("max usage")) {
      log("pnl_fallback", `getLatestSig: primary RPC 429, trying fallback`);
      const fb = getPnlFallbackConnection();
      const sigs = await fb.getSignaturesForAddress(new PublicKey(addr), { limit: 1 });
      return sigs?.[0]?.signature ?? null;
    }
    return null;
  }
...
async function getMeteoraData(conn, walletAddress, flat) {
  ...
  await Promise.all([...positionsByPool.entries()].map(async ([pool, positionAddrs]) => {
    ...
    const sigByPosition = {};
    await Promise.all(positionAddrs.map(async (addr) => { sigByPosition[addr] = await getLatestSigWithFallback(conn, addr); }));
...
function buildPosition(f, prices, solUsd, meteora, solMode) {
  ...
  const rawDepositsSol = safeNum(meteora?.allTimeDeposits?.total?.sol);
  const rawDepositsUsd = safeNum(meteora?.allTimeDeposits?.total?.usd);
  const tokenYSolAmount = safeNum(meteora?.allTimeDeposits?.tokenY?.amount);
  const depositsSol = solMode
    ? (rawDepositsSol > 0
        ? rawDepositsSol
        : (tokenYSolAmount > 0
            ? tokenYSolAmount
            : (rawDepositsUsd > 0 && solUsd > 0 ? rawDepositsUsd / solUsd : 0)))
    : 0;
  const depositsUsd = rawDepositsUsd;

  const rawWithdrawSol = safeNum(meteora?.allTimeWithdrawals?.total?.sol);
  const rawWithdrawUsd = safeNum(meteora?.allTimeWithdrawals?.total?.usd);
  const tokenYWithdrawAmount = safeNum(meteora?.allTimeWithdrawals?.tokenY?.amount);
  const withdrawSol = solMode
    ? (rawWithdrawSol > 0
        ? rawWithdrawSol
        : (tokenYWithdrawAmount > 0
            ? tokenYWithdrawAmount
            : (rawWithdrawUsd > 0 && solUsd > 0 ? rawWithdrawUsd / solUsd : 0)))
    : 0;
  const withdrawUsd = rawWithdrawUsd;

  const rawClaimedSol = safeNum(meteora?.allTimeFees?.total?.sol);
  const rawClaimedUsd = safeNum(meteora?.allTimeFees?.total?.usd);
  const tokenYClaimedAmount = safeNum(meteora?.allTimeFees?.tokenY?.amount);
  const claimedSol = solMode
    ? (rawClaimedSol > 0
        ? rawClaimedSol
        : (tokenYClaimedAmount > 0
            ? tokenYClaimedAmount
            : (rawClaimedUsd > 0 && solUsd > 0 ? rawClaimedUsd / solUsd : 0)))
    : 0;
  const claimedUsd = rawClaimedUsd;
  ...
  // Build a short human-readable pair name. Tracked pool_name wins.
  // Fallback: short mint prefix (e.g. "C1mg..Eory-SOL") instead of full address.
  const shortMint = (addr) => (addr && typeof addr === "string" && addr.length > 12)
    ? `${addr.slice(0, 4)}..${addr.slice(-4)}`
    : (addr || "?");
  const yDisplay = (meteora?.tokenY === SOL_MINT_STR || meteora?.tokenY === "SOL")
    ? "SOL"
    : shortMint(meteora?.tokenY);
  const pair = tracked?.pool_name
    || (meteora ? `${shortMint(meteora.tokenX)}-${yDisplay}` : "?/SOL");
...
export async function computePositions(walletAddress) {
  ...
  let conn = getPnlConnection();
  const DLMM = await loadDlmmSdk();

  let map;
  try {
    map = await DLMM.getAllLbPairPositionsByUser(conn, new PublicKey(walletAddress));
  } catch (err) {
    const is429 = err?.message?.includes("429") || err?.message?.includes("max usage");
    if (is429) {
      log("pnl_fallback", `computePositions: Helius 429 on getAllLbPairPositionsByUser, switching to fallback RPC`);
      conn = getPnlFallbackConnection();
      map = await DLMM.getAllLbPairPositionsByUser(conn, new PublicKey(walletAddress));
    } else {
      throw err;
    }
